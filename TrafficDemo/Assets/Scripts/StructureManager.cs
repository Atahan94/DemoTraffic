...
public class StructureManager : MonoBehaviour
{
    ...
    internal void PlaceBigStructure(Vector3Int position)
    {
        ...
        if(CheckBigStructure(position, width , height))
        {
            ...
            placementManager.PlaceObjectOnTheMap(position, bigStructuresPrefabs[randomIndex].prefab, CellType.Structure, width, height, randomIndex);
            AudioPlayer.instance.PlayPlacementSound();
        }
    }
    ...
    internal void PlacedLoadedStructure(Vector3Int position, int buildingPrefabIndex, CellType buildingType)
    {
        switch (buildingType)
        {
            case CellType.Structure:
                    placementManager.PlaceObjectOnTheMap(position, housesPrefabe[buildingPrefabIndex].prefab,
                    CellType.Structure, buildingPrefabIndex: buildingPrefabIndex);
                break;
            case CellType.SpecialStructure:
                placementManager.PlaceObjectOnTheMap(position, specialPrefabs[buildingPrefabIndex].prefab,
                   CellType.SpecialStructure, buildingPrefabIndex: buildingPrefabIndex);
                break;
